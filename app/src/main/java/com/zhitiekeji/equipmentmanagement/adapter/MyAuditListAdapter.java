 ...
 */
public class MyAuditListAdapter extends BaseAdapter {
    private Context context;
    private List<MyAuditListModel> list;
    private int selectIndex = 0;

    public MyAuditListAdapter(Context context, List<MyAuditListModel> list) {
        super();
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            convertView = LayoutInflater.from(context).inflate(R.layout.item_myauditlist, null);
        ...
        if (list.get(position).getFacility().getType().equals("1")){//判断是房屋还是构筑物
            ...
            holder.textView5.setText(list.get(position).getFacility().getRoof_area()+"㎡");//构筑物面积
        }
//        holder.textView6.setText(list.get(position).getFacility().getNumber());//批复号
        ...
        MyAuditListModel.FacilityDiseaseListBean model = new MyAuditListModel.FacilityDiseaseListBean();
        final List<MyAuditListModel.FacilityDiseaseListBean> list1 = new ArrayList<MyAuditListModel.FacilityDiseaseListBean>();
        ...
        if (list.get(position).getSelect() == 1){
            holder.imageView1.setImageResource(R.mipmap.xuanzhong);
        }else {
            holder.imageView1.setImageResource(R.mipmap.weixuan);
        }
...
//        holder.textView8.setText(list.get(position).getFacility().getNumber());//未通过理由
    ...
    private static class ViewHolder {
        ImageView imageView, imageView_more,imageView1;
